...
md"""
Lagrange Interpolation is an answer to the question: how do we create a function that goes through a number of points in the smoothest way possible? For some definition of "smooth".
"""
...
function interpolateLinear(x, upsampleFactor)
	N = length(x)
	Nup = N * upsampleFactor - 1;
	ys = zeros(Nup)
	for m in 1:Nup
		fracIdx = m / upsampleFactor + 1;
		n = Int(floor(fracIdx))
		c = fracIdx - n
		if (n + 1 <= N)
			ys[m] = (1 - c) * x[n] + c * x[n + 1]
		else
			ys[m] = (1 - c) * x[n]
		end
	end
...
# ╔═╡ 2f5f9fe1-b815-4363-8290-0f8ba94fe3ca
# there's another way to look at interpolation - as an FIR filter using a "basis function"
# lets create this triangle function:
function triangleBasis(upsampleFactor)
	len = upsampleFactor * 2 + 1
	midPoint = upsampleFactor
	w = zeros(len)
	for n in 1:len
		if (n <= midPoint)
			w[n] = (n - 1) / upsampleFactor;
		else
			w[n] = 2 - (n - 1) / upsampleFactor;
		end
	end
	w
end

# ╔═╡ 531b94a0-3f27-4d2b-a0c3-a3369b8b8ea4
stemPlot(triangleBasis(8))

# ╔═╡ 9828faa2-5e4f-4933-9819-ff9ebca7feea
function decimate(signal, D)
	out = []
	for s in signal
		out = [out; s; zeros(D - 1)] 
	end
...
# This is a "decimated" signal. 
stemPlot(decimate(x, upsampleFactor))
